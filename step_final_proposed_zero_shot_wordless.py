"""
Visual-only zero-shot graph-based PatentCLIP classification for IMPACT.

Each patent is represented by the visual portion of its semantic graph in
`detection_graph_json`. Title and keyword nodes are excluded before graph
tensor construction, so they do not participate in message passing or pooling.

Zero-shot requirement
---------------------
No IMPACT labels are used to train model parameters. This script therefore
uses no trainable GNN. All graph operations are parameter-free.

The script uses:
1. Frozen PatentCLIP image encoders for visual graph-node features.
2. Parameter-free relation-aware message passing.
3. Parameter-free type-balanced visual graph pooling.
4. Frozen descriptor-prompt text embeddings.
...
Excluded sample nodes
---------------------
- title nodes
- keyword nodes

Only usable visual nodes are inserted into graph tensors. Consequently,
excluded textual nodes and all edges incident to them are absent from the
zero-shot graph encoder.

Visual graph relations
----------------------
- contains_top_level_bbox
- contains_bbox

Reverse relations are added automatically so visual information can propagate
in both directions.

The class labels are still converted into textual descriptor prompts, for
example `a patent drawing of {descriptor}.`. These prompts define the frozen
zero-shot classifier and are not sample title or keyword nodes.
...
# Type-balanced pooling first averages figure nodes and bbox nodes
# separately, then averages the available visual type representations.
# This prevents numerous bbox nodes from overwhelming figure nodes.
...
def load_and_split():
    ...
    # Only a valid semantic graph is required. The CSV keywords column is not
    # used because keyword nodes are deliberately excluded from this variant.
    graph_mask = dataframe[GRAPH_COLUMN].apply(has_value)

    filtered_dataframe = (
        dataframe[graph_mask]
        .copy()
    ...
    print(
        "Rows after visual-graph + rare-class filtering: "
...
def collect_graph_requests(
    ...
    """Collect only visual embedding requests from each semantic graph."""
    ...
    for _, row in dataframe.iterrows():
        ...
        for node_id, node_data in graph.nodes(
            ...
            # title and keyword nodes are intentionally ignored.
    ...
    print("Sample text nodes encoded: 0")
...
def build_graph_tensors(
    ...
    """
    Build tensors from usable figure and bbox nodes only.

    Title/keyword nodes are not represented by zero vectors; they are absent
    from the node list, membership tensor, propagation graph, and pooling.
    """
    ...
    for node_id, node_data in graph.nodes(
        ...
        # No title/keyword branch exists in this visual-only variant.
        if feature is not None:
            kept_node_ids.append(node_id)
            visual_node_features.append(
                feature.float()
            )
            visual_node_types.append(
                NODE_TYPE_TO_INDEX[node_type]
            )
    ...
    node_id_to_index = {
        node_id: index
        for index, node_id in enumerate(kept_node_ids)
    }
    ...
    for source, destination, edge_data in graph.edges(
        ...
        # This rejects every edge touching an excluded text node or an
        # unusable visual node because such endpoints are absent from the map.
        if (
            source not in node_id_to_index
            or destination not in node_id_to_index
        ):
            continue

        source_index = node_id_to_index[source]
        destination_index = node_id_to_index[destination]
        ...
        reverse_relation = f"{relation}_rev"
        edge_lists[reverse_relation][0].append(
            destination_index
        )
        edge_lists[reverse_relation][1].append(
            source_index
        )
    ...
    for relation in ALL_RELATIONS:
        sources, destinations = edge_lists[relation]
...
class PatentGraphDataset(Dataset):
    def __init__(
        ...
        print(
            f"Built {len(self.items)} visual graphs; "
...
def main():
    ...
    print(
        ...
        + (
            "figure/bbox type-balanced mean pooling"
            if TYPE_BALANCED_POOLING
    ...
    print(
        "\nFinal visual-only zero-shot PatentCLIP graph results"
    )
