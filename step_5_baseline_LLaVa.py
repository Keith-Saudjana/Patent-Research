...
if len(sys.argv) != 3:
    ...
    print("Example: python script.py Impact_2022_Sub_Captions.csv ./locarno_llava_lora_Captions")
    sys.exit(1)
...
# --- Change 1: Use captions column instead of Qwen ---
...
# --- Change 2: Prompt uses caption paragraph instead of triples ---
def build_prompt(caption: str, for_training: bool = False, label: Optional[int] = None) -> str:
    categories_block = "\n".join(f"  {v}" for v in ID_TO_CATEGORY.values())

    if caption and caption.strip():
        caption_section = (
            f"Below is a textual description of a design patent:\n"
            f"{caption.strip()}\n\n"
            f"The image above is the visual representation of the same patent.\n\n"
            f"Using BOTH the image and the description, classify this patent into one of the following Locarno categories:\n"
        )
    else:
        caption_section = (
            f"No textual description is available for this patent.\n\n"
...
# --- Change 3: Dataset uses caption string instead of triples list ---
@dataclass
class PatentSample:
    image_path: str
    caption: str
...
class LocarnoPatentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt = build_prompt(sample.caption, for_training=self.is_train, label=sample.label)
...
# --- Change 4: Callback uses caption instead of triples ---
class GenerationEvalCallback(TrainerCallback):
    ...
    def on_epoch_end(self, args, state, control, model=None, **kwargs):
        ...
        for _, row in tqdm(self.eval_df.iterrows(), total=len(self.eval_df), desc="Gen-Eval"):
            ...
            prompt = build_prompt(caption, for_training=False)
            encoding = self.processor(
                text=prompt, images=image,
...
def train_lora_llava(
    ...
    gen_eval_callback = GenerationEvalCallback(
        eval_df=val_df, processor=processor,
    ...
    trainer = Trainer(
        ...
        callbacks=[CleanLogCallback(), gen_eval_callback],   # <-- add CleanLogCallback
    )
    ...
    gen_eval_callback.on_epoch_end(
        args=training_args,
        state=type("S", (), {"epoch": "final"})(),
        control=None,
        model=model
    )
