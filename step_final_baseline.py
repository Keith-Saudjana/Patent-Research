...
df_clean = df[["image_paths", "caption", "main_class"]].dropna()
df_clean["image_path"] = df_clean["image_paths"].apply(lambda x: x[0] if isinstance(x, list) else x)
...
class PatentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.tokenizer([caption]).squeeze(0) if caption else None
...
# --- Train until divergence ---
def train(
    model,
    train_df, val_df, test_df,
    output_dir,
    max_epochs=500,       # high ceiling — early stopping will trigger first
    batch_size=32,
    lr=1e-3,
    patience=5,           # epochs to wait before declaring divergence
    min_delta=1e-4,       # minimum improvement in val accuracy to count as progress
):
    ...
    # ReduceLROnPlateau adapts LR without needing a fixed T_max
    ...
    print(f"\n[Early Stopping] patience={patience}, min_delta={min_delta}, max_epochs={max_epochs}")

    for epoch in range(1, max_epochs + 1):
        model.train()
        total_loss = 0.0

        for images, tokens, labels in tqdm(train_loader, desc=f"Epoch {epoch}"):
            images = images.to(device)
            tokens = tokens.to(device) if tokens is not None else None
        ...
        if val_acc > best_acc + min_delta:
            ...
            print(f"  [Early Stop] No improvement for {epochs_no_improve}/{patience} epochs "
                  f"(best={best_acc:.4f}, current={val_acc:.4f})")

            if epochs_no_improve >= patience:
                print(f"\n[Divergence detected] Stopping at epoch {epoch}. "
