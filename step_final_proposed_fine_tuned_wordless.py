...
Important
---------
- No whole-patent image branch is used.
- No title-node or keyword-node feature enters the GNN.
- No edge connected to a removed textual node enters the GNN.
- No separate linear classification head is used.
- PatentCLIP is frozen. Only the visual graph encoder and graph projection train.
...
CHECKPOINT_PATH = os.path.join(
    CHECKPOINT_DIR,
    "patentclip_visual_graph_prompt_classifier.pth",
)
...
# Only relations whose endpoints can both be visual nodes are retained.
BASE_RELATIONS = [
...
def load_and_split_data():
    ...
    # This visual-only variant does not require the CSV keywords column.
    mask = dataframe[GRAPH_COLUMN].apply(has_value)
...
def collect_graph_requests(
    ...
    """
    Parse every graph once and collect only visual PatentCLIP requests.

    Title and keyword nodes remain in the stored NetworkX graph temporarily
    so bbox-to-figure path resolution can inspect the original schema. They
    are excluded later when graph tensors and edges are constructed.
    """
    ...
    for _, row in dataframe.iterrows():
        ...
        # Store the current node ID for the existing bbox path fallback.
        for node_id in graph.nodes:
            graph.nodes[node_id]["_node_id"] = node_id
        ...
        for _, node_data in graph.nodes(data=True):
            ...
            if node_type == "figure":
                ...
                if image_path is not None and bbox is not None:
                    crop_requests.append((image_path, bbox))

            elif node_type in {"title", "keyword"}:
                ignored_text_node_count += 1

    print(f"Parsed graphs: {len(graphs_by_row_id)}")
    print(f"Invalid graphs skipped: {invalid_graph_count}")
    print(
        "Text nodes excluded from GNN input: "
        f"{ignored_text_node_count}"
    )
    print(
...
def build_graph_tensors(
    ...
    # Strictly remove textual nodes from the graph seen by the GNN.
    node_ids = [
        node_id
        for node_id, node_data in graph.nodes(data=True)
        if node_data.get("node_type") in VISUAL_NODE_TYPES
    ]
    ...
    for node_id in node_ids:
        ...
        node_type_ids[node_index] = NODE_TYPE_TO_INDEX[node_type]
        feature = None
    ...
    # A graph with no usable visual PatentCLIP feature cannot be classified.
    ...
    for source, destination, edge_data in graph.edges(data=True):
        # This endpoint test removes every edge connected to title/keyword nodes.
        if (
        ...
        edge_lists[relation][0].append(source_index)
        edge_lists[relation][1].append(destination_index)

        reverse_relation = f"{relation}_rev"
        edge_lists[reverse_relation][0].append(destination_index)
        edge_lists[reverse_relation][1].append(source_index)

    edge_index_dictionary = {}

    for relation in ALL_RELATIONS:
        source_list, destination_list = edge_lists[relation]

        edge_index_dictionary[relation] = (
            torch.tensor(source_list, dtype=torch.long),
            torch.tensor(destination_list, dtype=torch.long),
        )
...
class PatentGraphDataset(Dataset):
    ...
    PatentCLIP visual-node features are already precomputed, so no image
    loading occurs inside the training loop.
    """
...
def main():
    ...
    # --------------------------------------------------------------
    # One-time visual graph node embedding
    # --------------------------------------------------------------
    all_rows = pd.concat(
        [
            training_dataframe,
            ...
            torch.save(
                {
                    ...
                    "visual_node_types": sorted(
                        VISUAL_NODE_TYPES
                    ),
                    "text_nodes_excluded": True,
                    "best_validation_accuracy": (
                        best_validation_accuracy
                    ),
                },
    ...
    print(
        "\nFinal PatentCLIP visual-graph-to-prompt results"
    )
    print(
        f"Test accuracy: "
